define(['app/app'], function (app) {
    ...
    /**
     * Usage: <link rel="stylesheet" ap-style-link />
     */
    app.directive('apStyleLink', [function () {
        var directive = {
            restrict: 'A',
            link: link
        };
        ...
        function link(scope, element, attrs) {
            attrs.$set('href', getStyleLink());

            function getStyleLink() {
                var hostUrlExpression = /^SPHostUrl=/i;
                var ctagExpression = /^SPClientTag=/i;
                var hostUrl = '';
                var ctag = '';
                var link = '';
                if (document.URL.indexOf('?') != -1) {
                    ...
                    for (var i = 0; i < params.length; i++) {
                        ...
                        if (hostUrlExpression.test(p)) {
                            hostUrl = p.split('=')[1];
                        } else if (ctagExpression.test(p)) {
                            ctag = p.split('=')[1];
                        }
                    }
                }
                if (hostUrl) {
                    link = hostUrl + '/_layouts/15/defaultcss.ashx';
                    if (ctag) {
                        link += '?ctag=' + ctag;
                    }
                } else {
                    link = '/_layouts/15/1033/styles/themable/corev15.css';
                }
    ...
    app.directive('ccSidebar', function () {
        // Opens and close the sidebar menu.
        // Usage:
        //  <div data-cc-sidebar>
        // Creates:
    ...
    */
    ...
    app.directive('ccWidgetHeader', function() {
        ...
        var directive = {
            ...
            templateUrl: '../scripts/app/layouts/widget.html',
            restrict: 'A'
        };
        return directive;
